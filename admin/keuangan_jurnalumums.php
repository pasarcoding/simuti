<?php if ($_GET[act] == '') { ?>
	...
	if (isset($_POST[update])) {
		...
		$query = mysqli_query($conn, "UPDATE jurnal_umum SET tgl='$waktu_sekarang',idPosBayar='$_POST[idPosBayar]', ket='$_POST[ket]',
							penerimaan='$_POST[penerimaan]',caraBayar='$_POST[caraBayar]',user='$_SESSION[namalengkap]' where id='$_POST[id]'");
		if ($query) {
	...
	if (isset($_POST['tambah'])) {
		...
		for ($i = 0; $i < count($idPosBayar); $i++) {
			$query = mysqli_query($conn, "INSERT INTO jurnal_umum(tgl,idPosBayar,ket,penerimaan,pengeluaran,caraBayar,user) VALUES('$waktu_sekarang','$idPosBayar[$i]','$ket[$i]','$penerimaan[$i]','0','$caraBayar[$i]','$_SESSION[namalengkap]')");
		}
