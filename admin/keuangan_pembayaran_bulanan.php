	...
	<div class="col-xs-4">
		<div class="box box-info box-solid">
			...
			<div class="box-footer">
				<a href="index.php?view=pembayaran&siswa=<?php echo $siswa; ?>&cari=Cari+Siswa" class="btn btn-primary pull-right"><span class="fa fa-reply"></span> Kembali</a>
			</div>
		</div>
	</div>
	<div class="col-xs-8">
				...
				<table class="table table-striped table-hover table-condensed">
					...
					<tbody>
						...
						if (isset($_GET['sms'])) {
							...
							if ($row['noHpOrtu'] == '') {
								echo "<script>document.location='index.php?view=bayarbulanan&jenis=$_GET[jenis]&tahun=$_GET[tahun]&siswa=$_GET[siswa]&err';</script>";
							} else {
								$userkey = "vmezut";
								...
								echo "<script>document.location='index.php?view=bayarbulanan&jenis=$_GET[jenis]&tahun=$_GET[tahun]&siswa=$_GET[siswa]&ok';</script>";
							}
...
<?php
	...
	if ($qHapus) {
		echo "<script>document.location='index.php?view=bayarbulanan&jenis=$_GET[idjenis]&tahun=$_GET[tahun]&siswa=$_GET[siswa]';</script>";
	} else {
		echo "<script>document.location='index.php?view=bayarbulanan&jenis=$_GET[idjenis]&tahun=$_GET[tahun]&siswa=$_GET[siswa]';</script>";
	}
	...
	if ($_GET['tipe'] == 'bulanan') {
		...
		$a = mysqli_query($conn, "SELECT nmBulan as Bulan, jenis_bayar.nmJenisBayar as jenis, nmSiswa as nama, noHpOrtu as hpo, noHpSis as hps, jumlahBayar as tagihan FROM siswa 
	...
	//	$jenis = $re['jenis'];
		...
		//pesan whatsapp ortu
		$number_wa[] = $hpo;
		$msg_wa[] =  "Assalamualaikum Wr Wb, Pembayaran Tagihan  $jenis untuk bulan $bulan sebesar *Rp." . rupiah($_GET[bayar]) . "* Peserta Didik a/n *$siswa*, telah kami terima *Lunas*. Terima kasih (Keuangan SD Muhammadiyah 3 Bandung)";
		//pesan whatsapp ortu
	//	$number_wa[] = $hps;
