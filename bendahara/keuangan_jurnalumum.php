<?php if ($_GET[act] == '') { ?>
	...
	if (isset($_POST[update])) {
		...
		$query = mysqli_query($conn, "UPDATE jurnal_umum SET tgl='$_POST[tgl]', ket='$_POST[ket]',
							idPosBayar='$_POST[idPosBayar]', pengeluaran='$_POST[pengeluaran]' where id='$_POST[id]'");
		if ($query) {
	...
	if (isset($_POST['tambah'])) {
		...
		for ($i = 0; $i < count($idPosBayar); $i++) {
			...
			$query = mysqli_query($conn, "INSERT INTO jurnal_umum(tgl,ket,idPosBayar,penerimaan,pengeluaran) VALUES('$tgl','$ket[$i]','$idPosBayar[$i]','0','$pengeluaran[$i]')");
		}
