	...
	while ($dj = mysqli_fetch_array($sqlJenisBayar)) {
		...
		if ($dj['tipeBayar'] == 'bulanan') {
			$sqlB = mysqli_query($conn, "SELECT
				...
				WHERE tagihan_bulanan.idJenisBayar='$dj[idJenisBayar]' AND tagihan_bulanan.statusBayar!='0' AND tagihan_bulanan.idKelas='$_GET[kelas]' AND (DATE(tagihan_bulanan.tglBayar) BETWEEN '$tgl1' AND '$tgl2') ORDER BY tagihan_bulanan.tglBayar ASC");
			$tot = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(jumlahBayar) AS totBayar
			FROM tagihan_bulanan_bayar
			INNER JOIN tagihan_bulanan ON tagihan_bulanan_bayar.idTagihanBulanan = tagihan_bulanan.idTagihanBulanan
			WHERE tagihan_bulanan.idJenisBayar='$dj[idJenisBayar]' AND tagihan_bulanan.statusBayar!='0' AND tagihan_bulanan.idKelas='$_GET[kelas]' AND (DATE(tagihan_bulanan_bayar.tglBayar) BETWEEN '$tgl1' AND '$tgl2')"));
		} else {
			$sqlB = mysqli_query($conn, "SELECT
