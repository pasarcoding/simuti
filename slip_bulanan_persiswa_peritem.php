				...
				<table class="table table-bordered table-striped">
					...
					<tbody>
						...
						while ($dj = mysqli_fetch_array($sqlJenisBayar)) {
							...
							WHERE idJenisBayar='$dj[idJenisBayar]' AND statusBayar<>'0' AND idSiswa='$_GET[siswa]' AND (DATE(tglBayar) BETWEEN '$tgl1' AND '$tgl2') ORDER BY urutan ASC");
					...
					WHERE tagihan_bebas.statusBayar<>'0' AND tagihan_bebas.idKelas='$_GET[kelas]' AND tagihan_bebas.idSiswa = '$_GET[siswa]' AND (DATE(tagihan_bebas_bayar.tglBayar) BETWEEN '$tgl1' AND '$tgl2')"));
						...
						$totBulanan = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(diBayar) AS totBayar FROM view_laporan_bayar_bulanan WHERE  idKelas='$_GET[kelas]' AND statusBayar<>'0' AND idSiswa='$_GET[siswa]' AND (DATE(tglBayar) BETWEEN '$tgl1' AND '$tgl2')"));
